	...
	$.fn.zyComment = function(options,param){
		...
		return this.each(function(){
			...
			this.createAgoCommentHtml = function(){
				...
				html += '	<div class="text" style="font-size:2rem;padding-bottom:10px;border-bottom: 1px solid #DFDFDF;"> 发表评论 </div>';
				html += '</div>';
				$(self).append(html);
				...
				$.each(para.agoComment, function(k, v){
					...
					item += '	<a class="avatar">';
					item += '		<img src="../../resources/img/foot.gif">';
					item += '	</a>';
			...
			this.createFormCommentHtml = function(){
				...
				boxHtml += '				<input type="text" id="userName" placeholder="请输入您的名称 (必填)"/>';
				boxHtml += '				<label class="userNameLabel" for="userName" ></label>';
				boxHtml += '			</div>';
				boxHtml += '			<div class="field" >';
				boxHtml += '				<input type="text" id="userEmail" placeholder="请输入您的E-mail (选填)"/>';
				boxHtml += '				<label class="userEmailLabel" for="userName"></label>';
				boxHtml += '			</div>';
				boxHtml += '		</div>';
				boxHtml += '		<div class="contentField field" >';
				boxHtml += '			<textarea id="commentContent" placeholder="请输入评论"></textarea>';
...
//						}
						$.ajax({
							url  : contextPath+"/comment/confim", 
							data  : {"id":articleId,"name":userName,"email":userEmail,"content":commentContent,"selfId":selfID},
							type : "GET",
...
//					};
					$.ajax({
						url  : contextPath+"/comment/confim", 
						data  : {"id":articleId,"name":userName,"email":userEmail,"content":commentContent,"selfId":0},
						type : "GET",
						success : function(result){
							if(result.resultCode == 0){
								alert("评论成功！");
